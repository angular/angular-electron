...
import {ElectronMessageBus} from '../shared/message_bus';
...
export class ElectronRootRenderer implements RootRenderer {
  ...
  private _componentRenderers: Map<string, ElectronRenderer> =
      new Map<string, ElectronRenderer>();
  private _windows: Map<string, Electron.BrowserWindow> =
      new Map<string, Electron.BrowserWindow>();

  constructor(
      messageBrokerFactory: ClientMessageBrokerFactory, bus: ElectronMessageBus,
      private _serializer: Serializer, public renderStore: RenderStore) {
    this._messageBroker = messageBrokerFactory.createMessageBroker(RENDERER_CHANNEL);
    bus.initChannel(EVENT_CHANNEL, false);
  ...
  createWindow(options:Electron.BrowserWindowOptions = {}){
    return {
  ...
  renderComponent(componentType: RenderComponentType): Renderer {
    ...
    if (!result) {
      result = new ElectronRenderer(this, componentType);
...
export class ElectronBrowserWindowRef {
  constructor(public electronAppRef){

  }
}

export class ElectronRenderer implements Renderer, RenderStoreObject {
  constructor(
      private _rootRenderer: ElectronRootRenderer, private _componentType: RenderComponentType) {}
...
class _AnimationWorkerRendererPlayer implements RenderStoreObject {
  ...
  constructor(private _rootRenderer: ElectronRootRenderer, private _renderElement: any) {}
